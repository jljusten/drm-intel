...
static int verify_hwconfig_blob(struct intel_guc *guc, struct intel_hwconfig *hwconfig)
{
	...
	if (!IS_ALIGNED(hwconfig->size, sizeof(u32))) {
		drm_notice(drm, "hwconfig blob size (%d) is not u32 aligned\n", hwconfig->size);
		return -EINVAL;
	}

	while (offset < hwconfig->size) {
		if (remaining < min_item_size) {
			drm_notice(drm, "hwconfig blob invalid (no room for item required fields at offset %lld)\n",
				   offset);
			return -EINVAL;
		}
		item_size = min_item_size + sizeof(u32) * item->length;
		if (item_size > remaining) {
			drm_notice(drm, "hwconfig blob invalid (no room for data array of item at offset %lld)\n",
				   offset);
			return -EINVAL;
		}
		offset += item_size;
...
static int guc_hwconfig_fill_buffer(struct intel_guc *guc, struct intel_hwconfig *hwconfig)
{
	...
	if (ret >= 0) {
		...
		if (verify_hwconfig_blob(guc, hwconfig)) {
			drm_notice(drm, "Ignoring invalid hwconfig blob received from GuC!\n");
			ret = -EINVAL;
		}
	}
